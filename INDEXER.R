...
setwd("W:/workspace/R") 
#
DBCONNECTION_PRODUCTION <- DBI::dbConnect(odbc::odbc(),
                                          driver            = "SQL Server",
...
FUNKTION_INDEXER <- function(DBCONNECTION_PRODUCTION){
...
#
#-------------------------------------------------------------------------------
# Begin der datensammlung für den dataframe der Messgroessen
#
# Ermitteln der notwendigen länge
#
TMP <- nrow(TMP_KMD_LG_MESSGROESSE)
TMP <- rep(NA,TMP)
#
# Initialisieren und befuellen der Vectoren
# Als Grundlage wird erstmal die sapalte lg_messgroesse genommen
# die Fläche sollte über den Dateinahmen ermittelt werden können
#
#TMP_st_bezeichnung       = 1:TMP
TMP_messgroesse           = TMP
TMP_mg_name               = TMP
TMP_mg_bem                = TMP
TMP_einheit_id            = TMP
...
TMP_untergrenze           = TMP
TMP_obergrenze            = TMP
TMP_lueckenersatz_bem     = TMP
TMP_lueckenersatz_code    = TMP
TMP_lueckenersatz_bez     = TMP
TMP_lueckenersatz_minuten = TMP
TMP_agg_prozent           = TMP
#
TMP_lg_messgroesse_id     = TMP_KMD_LG_MESSGROESSE$lg_messgroesse_id
TMP_messgroesse_id        = TMP_KMD_LG_MESSGROESSE$messgroesse_id
TMP_lg_messgroesse        = TMP_KMD_LG_MESSGROESSE$lg_messgroesse
#
TMP_lg_mg_name            = TMP_KMD_LG_MESSGROESSE$lg_mg_name
#
# Verarbeitung von TMP_KMD_MESSGROESSE
for (a in TMP_KMD_MESSGROESSE$messgroesse_id) {
  TMP_messgroesse[which(TMP_messgroesse_id           == a)] = TMP_KMD_MESSGROESSE$messgroesse[a]
  TMP_mg_name[which(TMP_messgroesse_id               == a)] = TMP_KMD_MESSGROESSE$mg_name[a]
  TMP_mg_bem[which(TMP_messgroesse_id                == a)] = TMP_KMD_MESSGROESSE$mg_bem[a]
  TMP_einheit_id[which(TMP_messgroesse_id            == a)] = TMP_KMD_MESSGROESSE$einheit_id[a]
  TMP_untergrenze[which(TMP_messgroesse_id           == a)] = TMP_KMD_MESSGROESSE$untergrenze[a]
  TMP_obergrenze[which(TMP_messgroesse_id            == a)] = TMP_KMD_MESSGROESSE$obergrenze[a]
  TMP_lueckenersatz_bem[which(TMP_messgroesse_id     == a)] = TMP_KMD_MESSGROESSE$lueckenersatz_bem[a]
  TMP_lueckenersatz_code[which(TMP_messgroesse_id    == a)] = TMP_KMD_MESSGROESSE$lueckenersatz_code[a]
  TMP_lueckenersatz_minuten[which(TMP_messgroesse_id == a)] = TMP_KMD_MESSGROESSE$lueckenersatz_minuten[a]
  TMP_agg_prozent[which(TMP_messgroesse_id           == a)] = TMP_KMD_MESSGROESSE$agg_prozent[a]
}
#
a = 0
# verarbeitung kmd_einheit
for(a in TMP_KMD_EINHEIT$einheit_id){
  TMP_einheit[which(TMP_einheit_id      == a)] = TMP_KMD_EINHEIT$einheit[a]
  TMP_einheit_name[which(TMP_einheit_id == a)] = TMP_KMD_EINHEIT$einheit_name[a]
  TMP_einheit_bem[which(TMP_einheit_id  == a)] = TMP_KMD_EINHEIT$einheit_bem[a]
}
#
a = 0
# Verarbeitung kmd_lueckenersatz
for (a in TMP_KMD_LUECKENERSATZ) {
  TMP_lueckenersatz_bez[which(TMP_lueckenersatz_code == a)] = TMP_KMD_LUECKENERSATZ$lueckenersatz_bez[a]
}
#
TMP <- nrow(TMP_KMD_LG_MESSGROESSE)
#
INDEX_EINHEITEN <- data.frame(
  st_bezeichnung        = TMP,
  lg_messgroesse_id     = TMP,
  lg_messgroesse        = TMP,
  messgroesse_id        = TMP,
  ...
  lueckenersatz_bez     = TMP,
  lueckenersatz_minuten = TMP,
  lueckenersatz_bem     = TMP,
  agg_prozent           = TMP
)
#
INDEX_EINHEITEN$lg_messgroesse_id     = TMP_messgroesse_id
INDEX_EINHEITEN$lg_messgroesse        = TMP_lg_messgroesse
INDEX_EINHEITEN$messgroesse_id        = TMP_messgroesse_id
INDEX_EINHEITEN$messgroesse           = TMP_messgroesse
INDEX_EINHEITEN$mg_name               = TMP_mg_name
INDEX_EINHEITEN$mg_bem                = TMP_mg_bem
INDEX_EINHEITEN$einheit_id            = TMP_einheit_id
INDEX_EINHEITEN$einheit               = TMP_einheit
INDEX_EINHEITEN$einheit_name          = TMP_einheit_name
INDEX_EINHEITEN$einheit_bem           = TMP_einheit_bem
INDEX_EINHEITEN$untergrenze           = TMP_untergrenze
INDEX_EINHEITEN$obergrenze            = TMP_obergrenze
INDEX_EINHEITEN$lueckenersatz_code    = TMP_lueckenersatz_code
INDEX_EINHEITEN$lueckenersatz_bez     = TMP_lueckenersatz_bez
INDEX_EINHEITEN$lueckenersatz_minuten = TMP_lueckenersatz_minuten
INDEX_EINHEITEN$lueckenersatz_bem     = TMP_lueckenersatz_bem
INDEX_EINHEITEN$agg_prozent           = TMP_agg_prozent
#
return
#
}

dbDisconnect(DBCONNECTION_PRODUCTION)