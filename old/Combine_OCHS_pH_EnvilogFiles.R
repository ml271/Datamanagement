...
#setwd("C:/test")
setwd(paste0("O:/PROJEKT/NIEDER/LOGGER/OCHS/Ochsenhausen_Fichte_ungedügt_pH/", vch.Year))
#-----------------------------------------------------------------------------------
#load all csv-file from directory
tbl <- list.files(pattern = "*.csv")
#tbl <- tbl[-c(21:24)]
tbl <-  tbl %>%  map_df( ~read.csv2( ., skip=1,
                                     check.names=FALSE, na.strings = c("(NOREPLY)","(NOVALUE)"),
...
tbl$'#17:% DH:(do.):ID(55)' <- round(as.numeric(comtodot(tbl$'#17:% DH:(do.):ID(55)')),2)
tbl$'#18:S/m DH:(do.):ID(55)' <- round(as.numeric(comtodot(tbl$'#18:S/m DH:(do.):ID(55)')),2)
tbl$'#22:pH Nu:(do.):ID(5)' <- round(as.numeric(comtodot(tbl$'#22:pH Nu:(do.):ID(5)')),2)
tbl$'#23:pH Nu:(do.):ID(25)'<- round(as.numeric(comtodot(tbl$'#23:pH Nu:(do.):ID(25)')),2)
...
l[110:115]
############ DATENLÜCKE #############
# eine Datenlücke vom "2020-06-29"  bis zum "2020-07-15" wurde gefunden und ncoh nicht behoben!
...
write.csv2(tbl2, file=paste0("OC_Level2FI_pH_Envilog__", vch.Year,"_combine.csv"), quote = F, row.names = F, na = "")
...
l <- read.csv2(paste0("W:/Datenbank Level2/Import DB/","Ochsenhausen_import_data/OC_Level2FI_Envilog__2020_combine.csv"), skip=1) %>% 
  mutate(Time = dmy_hm(Time)) %>%
  mutate(Date = date(df$Time)) %>%
  distinct(Date) %>% pull()
t <- seq.Date(from = ymd(paste0(vch.Year, "-01-01")), to = ymd(paste0(vch.Year, "-12-31")), by = 1)
l == t
l[180:183]
